...
class ToDoKafkaResourceIT {
    ...
    void setup() {
        ...
        ToDoKafkaResource kafkaResource = new ToDoKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/to-do-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/to-do-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
